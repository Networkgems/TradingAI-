...
import { summarizeLadderCeiling, requiredFillRate, BREAKEVEN_MAKER_RECOVERY, } from '../packages/server/src/option-maker-shadow.js';
...
function main() {
    ...
    const pools = new Map(SLEEVES.map((s) => [s.structure, []]));
    let files = 0;
    let rowsSeen = 0;
    for (const path of chainFiles()) {
        files += 1;
        const chain = JSON.parse(readFileSync(path, 'utf-8'));
        for (const row of chain.rows ?? []) {
            rowsSeen += 1;
            const bid = row.bid ?? 0;
            const ask = row.ask ?? 0;
            if (!(bid > 0) || !(ask > 0) || ask < bid)
                continue; // two-sided, sane book only
            const mark = (bid + ask) / 2;
            if (!(mark > 0))
                continue;
            const spreadPct = (ask - bid) / mark;
            for (const sleeve of SLEEVES) {
                if (spreadPct <= sleeve.maxSpreadPct)
                    pools.get(sleeve.structure).push({ bid, ask });
            }
        }
    }
    console.log(`\nTRA-1662 — MAKER LADDER CEILING (recorded chains)`);
    console.log(`files=${files}  rows scanned=${rowsSeen.toLocaleString()}`);
    console.log(`ladder: fractions=[${config.fractions.join(', ')}]  stepWait=${config.stepWaitMs}ms  ` +
    ...
    for (const sleeve of SLEEVES) {
        ...
        const meanSpreadPct = quotes.reduce((a, q) => a + (q.ask - q.bid) / ((q.ask + q.bid) / 2), 0) / quotes.length;
        console.log(`── ${sleeve.structure} ──────────────────────────────────────────`);
        console.log(`  admissible quotes (n)        ${ceiling.n.toLocaleString()}`);
        console.log(`  mean round-trip cross        ${pct(meanSpreadPct)} of premium  ` +
        ...
        console.log(`  ladder's max possible        ${pct(ceiling.maxAchievableRecoveryPct)}` +
        ...
        for (const r of ceiling.rungs) {
            const label = r.rung === 0
                ? 'mid + 1 tick'
                : `mid + ${(config.fractions[r.rung] * 100).toFixed(0)}% × hs`;
            const need = r.requiredFillRateForBreakeven === null
                ? 'UNREACHABLE (even at 100% fills)'
                : pct(r.requiredFillRateForBreakeven);
            console.log(`   ${r.rung}   │ ${label.padEnd(15)} │ ${pct(r.avgRecoveryPctIfFilled).padStart(18)} │ ${need}`);
        }
        ...
        const best = ceiling.rungs.reduce((a, b) => b.avgRecoveryPctIfFilled > a.avgRecoveryPctIfFilled ? b : a);
        ...
        for (const mark of [0.5, 1, 1.5, 2, 3, 5, 10]) {
            ...
            console.log(`  $${mark.toFixed(2).padEnd(6)} │ ${(hs / config.tickSize).toFixed(1).padStart(6)} ticks │ ` +
...
main();
//# sourceMappingURL=measure-maker-ladder-ceiling.mjs.map