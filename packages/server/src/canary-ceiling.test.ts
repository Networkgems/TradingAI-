// TRA-3836 (parent TRA-3827) — the <=$100 attended-canary ceiling.
//
...
describe('resolveCanaryCeiling', () => {
  it('unset env resolves the compiled $100 default — the control is code-only armed', () => {
    const c = resolveCanaryCeiling({});
    expect(c).toEqual({ perOrderUsd: 100, aggregateUsd: 100, source: 'compiled_default' });
    expect(CANARY_CEILING_DEFAULT_USD).toBe(100);
    expect(CANARY_CEILING_HARD_MAX_USD).toBe(100);
  });

  it('env may LOWER the ceiling', () => {
    const c = resolveCanaryCeiling({ [CANARY_CEILING_VAR]: '50' });
    expect(c).toEqual({ perOrderUsd: 50, aggregateUsd: 50, source: 'env' });
  });

  it('env above the hard max clamps DOWN to $100 — no env value widens', () => {
    const c = resolveCanaryCeiling({ [CANARY_CEILING_VAR]: '750' });
    expect(c?.perOrderUsd).toBe(100);
    expect(c?.aggregateUsd).toBe(100);
  });
...
describe('gradeCanaryCeiling', () => {
  ...
  it('an exactly-$100 order on a flat book passes (<=, not <)', () => {
    expect(gradeCanaryCeiling(100, pricedFlat, ceiling).allowed).toBe(true);
  });

  it('POSITIVE CONTROL — the first 2026-08-17 fill ($272 premium) is refused per-order', () => {
    const v = gradeCanaryCeiling(272, pricedFlat, ceiling);
    expect(v.allowed).toBe(false);
    expect(v.reasonCode).toBe('canary_ceiling_per_order');
  });

  it('POSITIVE CONTROL — the second 08-17 fill ($419) is refused on BOTH scopes; per-order rules first', () => {
    const v = gradeCanaryCeiling(419, { usd: 272, rows: 1, unpricedRows: 0 }, ceiling);
    expect(v.allowed).toBe(false);
    expect(v.reasonCode).toBe('canary_ceiling_per_order');
  });

  it('a pair of $90 orders is caught by the AGGREGATE arm — why one cap alone is not enough', () => {
    const first = gradeCanaryCeiling(90, pricedFlat, ceiling);
    expect(first.allowed).toBe(true);
    const second = gradeCanaryCeiling(90, { usd: 90, rows: 1, unpricedRows: 0 }, ceiling);
...
describe('gradeCanaryCeilingHealth', () => {
  ...
  it('POSITIVE CONTROL — the 08-17 book publishes a NEGATIVE residual and a breach verdict', () => {
    ...
    // SIGNED: -$591, not the floored 0 that let 6.91x read as "within".
    expect(h.books?.[0]?.signedResidualUsd).toBe(-591);
  });

  it('a flat armed book reads within with the full $100 residual', () => {
    ...
    expect(h.books?.[0]?.signedResidualUsd).toBe(100);
  });
