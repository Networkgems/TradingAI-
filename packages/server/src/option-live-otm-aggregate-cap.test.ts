...
describe('foldOpenPremiumAtRisk (TRA-3445)', () => {
  it('sums premiumPaid × contractsRemaining × 100 over the open rows', () => {
    expect(foldOpenPremiumAtRisk([pos()]))
      .toEqual({ usd: 150, rows: 1, unpricedRows: 0, adoptedUsd: 0, adoptedRows: 0, attributionBlindRows: 0, operatorPinnedUsd: 0, operatorPinnedRows: 0, unbookedEntryPremiumUsd: 0, unbookedEntryPremiumRows: 0, unbookedEntryPremiumSuppressedUsd: 0 });
    expect(foldOpenPremiumAtRisk([
      pos({ id: 'a' }),
      pos({ id: 'b', premiumPaid: 0.82 }),
      pos({ id: 'c', premiumPaid: 2.00, contracts: 2, contractsRemaining: 2 }),
    ])).toEqual({ usd: 150 + 82 + 400, rows: 3, unpricedRows: 0, adoptedUsd: 0, adoptedRows: 0, attributionBlindRows: 0, operatorPinnedUsd: 0, operatorPinnedRows: 0, unbookedEntryPremiumUsd: 0, unbookedEntryPremiumRows: 0, unbookedEntryPremiumSuppressedUsd: 0 });
  });
  ...
  it('COUNTS an unusable row rather than folding it to zero — the figure is known-low', () => {
    ...
    expect(fold).toEqual({ usd: 150, rows: 1, unpricedRows: 3, adoptedUsd: 0, adoptedRows: 0, attributionBlindRows: 0, operatorPinnedUsd: 0, operatorPinnedRows: 0, unbookedEntryPremiumUsd: 0, unbookedEntryPremiumRows: 0, unbookedEntryPremiumSuppressedUsd: 0 });
  });
  ...
  it('an EMPTY book is $0 at risk with zero rows — not a null or a synthetic bucket', () => {
    expect(foldOpenPremiumAtRisk([]))
      .toEqual({ usd: 0, rows: 0, unpricedRows: 0, adoptedUsd: 0, adoptedRows: 0, attributionBlindRows: 0, operatorPinnedUsd: 0, operatorPinnedRows: 0, unbookedEntryPremiumUsd: 0, unbookedEntryPremiumRows: 0, unbookedEntryPremiumSuppressedUsd: 0 });
  });
});
