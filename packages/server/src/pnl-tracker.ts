...
export class PnlTracker {
  ...
  /** When the calendar day has changed, record today's opening values. */
  ...
  /** Call this after every trade close to keep equity state durable. */
  ...
  /** Call once at EOD to record the day's closing stats. */
  saveSnapshot(snapshot: DailySnapshot): void {
    ...
    // Advance opening for the next day
    this.state.openingEquity = snapshot.closingEquity;
    this.state.openingOptionsPnl = this.state.optionsPnl;
    this.state.openingDate = snapshot.date; // will advance on next call to constructor/advanceDayIfNeeded
    this.persistState();
  }
  ...
  getCumulativeStats(currentEquity: number): CumulativeStats {
    ...
    return {
      allTimePnl: currentEquity - INITIAL_EQUITY,
