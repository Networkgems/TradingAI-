...
import { journalIdForPosition, type OptionTradeJournalRecord } from './option-trade-journal.js';
...
export type ExpiredDemoOrphanTreatment = 'void' | 'held_expired' | 'live' | 'unparseable';
...
export interface ExpiredDemoOrphanPlan {
  ...
  demoOpenRows: number;
  /** Expired and held by no book — the alarm number, and what gets voided. */
  orphans: number;
  /** Expired but a book still holds the position. Never voided. */
  heldExpired: number;
  /** Contract not yet expired. */
  live: number;
  /** No OCC to read an expiry from (multi-leg, legacy import). */
  unparseable: number;
...
export function planExpiredDemoOrphans(
  ...
  for (const r of rows) {
    ...
    plan.demoOpenRows += 1;
    const expiry = occExpiryDate(r.optionSymbol);
    ...
    if (treatment === 'void' || treatment === 'held_expired') {
      plan.rows.push({
        id: r.id,
        symbol: r.symbol,
...
export async function runExpiredDemoOrphanSweep(
  ...
  try {
    ...
    for (const row of rows) {
      ...
      const ok = await deps.recordVoid(row.id, EXPIRED_DEMO_ORPHAN_REASON, row.account);
      if (ok) {
