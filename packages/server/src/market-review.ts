...
function renderMarkdown(review: MarketReview): string {
  ...
  lines.push(`> \`${SPX_SYMBOL}\` / \`${NDX_SYMBOL}\` / \`${VIX_SYMBOL}\` / \`${TNX_SYMBOL}\`.`);
  lines.push('');
  // TRA-3436 — state the provenance in the artifact. These four series come from
  // a DIFFERENT feed than the one the engine trades on, and none of them is in
  // the engine's quote universe at all, so a reader who reconciles this table
  // against `/api/state` is comparing two numbers that were never meant to
  // agree. That reconciliation was attempted (TRA-3436) and cost real time.
  //
  // Deliberately carries NO field values and names no vendor-specific status —
  // nothing here can go stale. The proxy substitution is already visible in the
  // table: the Index column prints the symbol the candles ACTUALLY came from, so
  // a row reading `SPY` or `QQQ` is a fallback day.
  lines.push(`> **Source:** Yahoo (\`${SPX_FALLBACK_SYMBOL}\`/\`${NDX_FALLBACK_SYMBOL}\` ETF proxies, then Tradier, when an`);
  lines.push(`> index feed is dark — the Index column names the series actually used).`);
  lines.push(`> This is NOT the engine's quote feed: these symbols are not in the traded`);
  lines.push(`> universe, so levels here will differ from \`/api/state\` in vendor and in`);
  lines.push(`> snapshot instant, and on a fallback day in instrument. Expected, not a fault.`);
  lines.push('');
