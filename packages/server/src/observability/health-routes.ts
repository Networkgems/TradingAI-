...
export function registerLiveHealthRoutes(app: Express, deps: LiveHealthDeps): void {
  ...
  app.get('/api/health/live-enforce-gates', (_req, res) => {
    ...
    // ── ⭐ TRA-4875 — THE DEGRADATION SURFACE (AC2) ───────────────────────────
    //
    // `arm.costBar.edge.freshness.inputStale` (TRA-4783) has read `true` since
    // roughly 2026-08-18 and NOTHING CONSUMED IT. The gate went on refusing
    // 11,855 of 11,855 live entry-site candidates off constants derived from a
    // tape that stopped advancing in early August, with no alert, no annotation
    // on the decision, and `ok: true` on this very route. It was found by hand
    // off a postmarket review five weeks later; the CTO's 2026-09-20
    // corroboration on TRA-4622 read the table's perfect day-to-day
    // reproducibility as evidence of ROBUSTNESS, when it was the signature of
    // the freeze.
    //
    // ⛔ THIS RAISES; IT DOES NOT ADMIT. A stale estimator must keep refusing —
    // the defect was the silence, not the refusal (TRA-4875 item 3). Nothing on
    // the admission path reads `degradations` or `ok`, and no gate, arm, bar or
    // reason code branches on either.
    //
    // ⚠️ SCOPED TO CELLS THAT ACTUALLY DECIDED SOMETHING, deliberately. The
    // global `inputTapeAgeDaysMax` folds ALL eight estimator cells and on
    // 2026-09-24 read 78.1 d off `0.40-0.45` — a cell the armed strike selector
    // (band `[0.25, 0.40)`) can never nominate — while the two cells that
    // refused the live entry site sat at 52.3 d and 51.1 d. Keying `ok` on the
    // global max would have flipped this route red for a cell no decision
        ...
        : [
            {
               ...
               * the case where they DISAGREE (global true off a cell nobody
               * nominates, every decision-relevant cell fresh) is visible
    ...
    // (hydrated from disk at boot through the same `apply()` the live path
    // uses), and with no field saying so three separate readers independently
    // concluded "since-boot" — the last of them one fire away from installing a
    // permanent carve-out discounting the current ET day on a live-money
    // compliance arm. The block is computed, not asserted: see
    // `describeLiveEnforceCounterProvenance` for the ordered discriminators and
    // for why the negative branches are deliberately asymmetric.
    const liveEnforceBuild = resolveBuildInfo();
    const counterProvenance = describeLiveEnforceCounterProvenance(summary, {
      scope: 'top_level',
      etDay,
    ...
    res.json({
      /**
       * ⭐ TRA-4875 — NO LONGER A LITERAL. `ok: false` means at least one entry
       * in `degradations` below; read that array for what and why. This is the
       * surface chosen to satisfy AC2 ("at least one surface outside
       * `arm.costBar.edge.freshness` reports the degradation").
       *
       * ⚠️ This is NOT the container health check — Render probes
       * `/api/health` (see `render.yaml: healthCheckPath`), which this cannot
       * reach. A `false` here pages a reader; it restarts nothing and darks
       * nothing.
       */
      ok: degradations.length === 0,
      /**
       * ⭐ TRA-4875 — every currently-detected degradation of the gate surface,
       * as data rather than as prose buried in `note`. Empty array = nothing
       * detected (which is a measurement, not an absence of instrumentation:
       * the detector runs on every request). READ-ONLY; no admission path
       * consults it.
       */
      degradations,
      time: new Date(nowMs).toISOString(),
      build: liveEnforceBuild,
      etDay,
      ...
      ...summary,
      /**
       * TRA-4879 — the PROVENANCE of `decisionsRecorded` and the `byGate[]`
       * immediately above. Read `counterProvenance.verdict` BEFORE reading any
       * count here as a since-boot rate.
       *
       * ⛔ The two counters on this block do NOT share a span, which is half of
       * why the missing label was guessed wrong: `decisionsRecorded` is the
       * total across ALL retained ET days, while `byGate[]` is the requested ET
       * day alone. `covers` spells both out per field.
       */
      counterProvenance,
      /**
       * TRA-4879 — and the same for `retained`, which the ask names explicitly.
       * The retained fold is hydrated from the identical on-disk store, so its
       * provenance verdict is the same measurement; only `covers` differs,
       * because this fold spans the retained window rather than one ET day.
       */
      retained: {
        ...summary.retained,
        counterProvenance: describeLiveEnforceCounterProvenance(summary, {
          scope: 'retained',
          processStartedAt: liveEnforceBuild.startedAt,
        }),
      },
  ...
  app.get('/api/health/option-real-fill-shadow', async (req, res) => {
    ...
    res.json({
      ...
      note:
        ...
        + 'TRA-4897 — schema is now `real_fill_shadow_v2` and the population is PARTITIONED. '
        + '⛔ `admitted` (we opened it; realised) and `refused` (the gate refused it; counterfactual, no '
        + 'open, no exit, no P&L) MUST NEVER BE POOLED, and there is deliberately no pooled aggregate in '
        + 'this payload to pool them with — every fold lives under `summary.byAdmission.{admitted,refused}` '
        + 'and the old top-level `summary.overall` is GONE. Read `summary.nAdmitted` / `summary.nRefused` '
        + 'first; when `nAdmitted` is 0, `summary.admittedEmptyReason` says what that does and does not mean. '
        + '⛔ A `cost_bar`-refused candidate is NOT "a trade we declined". `cost_bar` is the highest-blocking '
        + 'gate on the live path and it `continue`s, so `spread`, `otm_delta_floor`, `aggregate_cap`, '
        + '`canary_ceiling`, `fleet_reachable_bound`, `exit_actionability` and `sleeve_stand_down` read '
        + '`evaluated: 0` on these rows — they were NEVER RUN. Never label it `declined` / `would_have_traded`. '
        + '⛔ GRADE THE REFUSED COHORT AGAINST `/api/health/option-maker-recovery` BEFORE PUBLISHING ANY '
        + 'RATE. That ledger already reads `single_leg_otm` attempts 817 / fills 590 / makerFillRate 0.722 / '
        + 'avgRecoveryPctAllAttempts −0.257 / breakeven 0.736 / `coversItsOwnSpread: false`, with '
        + '`avgFillRung 3.83` of [0,0.25,0.5,0.75,1] — real fills cluster at the ASK, not the mid. This '
        + 'route\'s print-through-at-mid rule is STRICTER, so a HIGH fill rate here is a finding about the '
        + 'RULE, not about the market. '
        + '⛔ NO EXECUTION IMPROVEMENT, INCLUDING A PERFECT ONE, ADMITS ANY CURRENT CELL. Drive '
        + '`spreadCrossR` to 0 and the bar is floor-pinned at `max(0.05+0+0.10, minGrossR 0.30) = 0.30`, '
        + 'while the live cells carry −0.215375 / −0.063103 / −0.051113 — a shortfall of ≥0.35R under FREE '
        + 'execution. This is instrumentation, not a route back to trading; 0.385R and the TRA-4750 '
        + 'stand-down are untouched. '
        + '⚠️ `byAdmission.*.sampling` is SINCE BOOT (`samplingWindow`, `bootedAt`) while the row counts '
        + 'beside it are DURABLE on a persistent disk. Do not divide one by the other. '
        + '⚠️ Refused rows are sized `refused_nominal_1` (the gate rejects before sizing), so '
