  ...
  app.get('/api/health/live-enforce-gates', (_req, res) => {
    ...
    const coverageClause =
      coverage.missingSessions.length > 0
        ...
        : `no expected session is absent through ${coverage.lastSessionEtDay ?? 'n/a (date outside the calendar bundle)'}`;
