...
export class DailyRiskGovernor {
   ...
   *
   * TRA-1435 — `giveBackArmFloor` is the minimum peak (book currency) the day
   * must reach before the GIVE-BACK cap can arm; the caller computes it from the
   * `BOOK_GIVEBACK_ARM_FLOOR_ENABLED` sub-flag (0 when off ⇒ legacy arm-at-any-
...
export class SignalEngine {
  ...
  private async sweepEquityEntryUniverse(activeSymbols: string[]): Promise<Array<{ sym: string; candles: Candle[] }>> {
    ...
    // TRA-1834 — key the funnel by (mode, engineId) so this engine's pass can never be
    // nulled by a sibling demo engine's gated tick. `feedContextKey` is the stable id.
    beginEquityEntryPass(this.mode, this.feedContextKey, { symbolsConsidered: activeSymbols.length });
  ...
  private async runOptionsExitPass(prices: Map<string, number>, pacer?: TickPacer): Promise<void> {
    ...
    const optionExitRisk = isExitRiskRulesEnabled(this.mode === 'live' ? process.env : this.resolveDemoFlagEnv()) || takeProfitEarlyArmed
      ...
      : undefined;
    // TRA-1418 (TRA-1417 build, parent TRA-1406 / TRA-1410 option a) — the
    // demo-only defined-risk combo exit policy. When the standalone
    // `ENABLE_OPTION_MULTILEG_EXIT` flag is armed, hand `checkExits` the exit
    // params so a demo combo gets a real per-tick net mark + WIN/LOSS resolution
    // instead of a $0 force-scratch. Scoped to `mode === 'demo'` so the LIVE combo
    // path is structurally untouched; absent → legacy blanket combo skip.
    const multiLegExitParams: MultiLegExitParams | undefined =
    ...
    const bookMarkEnv = this.mode === 'live' ? process.env : this.resolveDemoFlagEnv();
    if (isExitRiskRulesEnabled(bookMarkEnv)) {
  ...
  private async openSma200Pullback(signal: Sma200Signal): Promise<void> {
    ...
    if (!this.isDeterministicAutoTradingEnabled()) {
      recordEquityEntryRejected(funnelMode, funnelEngineId, 'auto_trading_disabled');
      return;
    }
    if (this.riskGovernor.isHalted()) {
      recordEquityEntryRejected(funnelMode, funnelEngineId, 'risk_halted');
      return;
    }
    ...
    if (!isStockMarketOpen()) {
      recordEquityEntryRejected(funnelMode, funnelEngineId, 'market_closed');
      return;
    }
  ...
  private async routeEquitySignal(
    ...
    if (this.equitySwingModeEnabled() && !isLiquidSwingSymbol(sym)) {
      ...
      recordEquityEntryRejected(funnelMode, funnelEngineId, 'swing_universe');
      return null;
    }
    ...
    if (recent) {
      recordEquityEntryRejected(funnelMode, funnelEngineId, 'recent_signal_dedup');
      return null;
    }
    ...
    if (pos) {
      ...
      recordEquityEntryRejected(funnelMode, funnelEngineId, 'sizing_returned_no_position');
    }
 ...
 * The add path submits REAL broker orders, so it was HARD-restricted to the
 * Tradier SANDBOX env (zero real capital) for evidence accrual while the named
 * promotion gates ran.
 *
 * TRA-971 / TRA-1597 (07-11) — the code gate is now OPEN. All promotion gates
 * are green: (1) demo forward evidence 456 adds / 0 R-cap breaches
 * (`/api/health/conviction-dca`), (2) QuantTrader live-$ nod GIVEN, (3) board
 * approval GRANTED (interaction `a1acc1a3`, `dca-971-live` = APPROVE). The two
 * originally-named conditions are substantively satisfied: CFO soak TRA-1393 =
 * `done`; TRA-382 chain-capture data gate = MET (38/30 trading days) — that
 * issue only rests `blocked` on a departed-agent ownership boundary, not a
 * substantive failure.
 *
 * NOTE the belt-and-suspenders is intentionally preserved: opening this
 * constant does NOT by itself submit real capital. Live add-orders still require
 * the second, independent switch — `liveEquityDcaAddsTradier = true` on the LIVE
 * Tradier AccountSettings (a runtime, per-account toggle). Both must be on.
 */
const LIVE_EQUITY_DCA_PRODUCTION_ENABLED = true;

/**
 * TRA-1439 — is the LIVE conviction-DCA equity add path permitted to submit
 * broker add-orders on `env`? The production gate is now OPEN
 * ({@link LIVE_EQUITY_DCA_PRODUCTION_ENABLED}, board-ratified TRA-971/TRA-1597),
 * so both `sandbox` and `production` envs are permitted at the ENV layer; actual
 * live submission still requires the independent `liveEquityDcaAddsTradier` arm
 * on the LIVE AccountSettings. Pure + exported so the gate is unit-testable
