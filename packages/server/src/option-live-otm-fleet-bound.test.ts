...
function bookRow(
  ...
  return {
    ...
    liveEntryGateOpen,
    availableCashUsd,
    capUsd: resolveLiveOptionTestBookAggregateCapUsd(availableCashUsd, A, PHI),
  };
}
...
describe('TRA-3723 — the fleet fail-open the per-book clamp cannot catch', () => {
  ...
  it("PER BOOK the docstring's promise is kept — no book can exceed A at any balance", () => {
    ...
    for (const cash of [400, 1143.96, 2000, 25_000, 10_000_000]) {
      expect(resolveLiveOptionTestBookAggregateCapUsd(cash, A, PHI)).toBeLessThanOrEqual(A);
    }
  });
