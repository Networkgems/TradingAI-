...
/**
 * TRA-307 — pulls 1m OHLCV bars from Coinbase Exchange's public candles
 * endpoint over `[fromMs, toMs)`, paginated in 300-bar windows (5 hours
 * each). Mirrors {@link fetchCoinbaseHourlyBars} so the scalping sweep has
 * the same retry / dedupe / sort semantics the 4H path already validated.
 */
export async function fetchCoinbase1mBars(
  symbol: string,
  fromMs: number,
  toMs: number,
): Promise<Candle[]> {
  if (toMs <= fromMs) return [];
  const stepMs = MAX_CANDLES_PER_REQUEST * GRANULARITY_1M * 1000;
  const all: Candle[] = [];
  ...
  while (cursor < toMs) {
    const winEnd = Math.min(cursor + stepMs, toMs);
    const url = new URL(`${COINBASE_BASE}/products/${symbol}/candles`);
    url.searchParams.set('granularity', String(GRANULARITY_1M));
    url.searchParams.set('start', new Date(cursor).toISOString());
    url.searchParams.set('end', new Date(winEnd).toISOString());

    const res = await fetch(url.toString(), { headers: { 'User-Agent': 'TRA-307/1.0' } });
    if (!res.ok) {
      const body = (await res.text()).slice(0, 200);
      throw new Error(`Coinbase ${res.status} for ${symbol} 1m candles: ${body}`);
    }
    const rows = (await res.json()) as ReadonlyArray<CoinbaseCandleRow>;
    for (const [time, low, high, open, close, volume] of rows) {
      all.push({ symbol, timestamp: time * 1000, open, high, low, close, volume });
