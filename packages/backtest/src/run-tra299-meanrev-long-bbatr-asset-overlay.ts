/**
 * TRA-299 — Phase-1.2.3 BB+ATR + asset-side EMA(50) overlay supplementing
 * SMA(50) BTC.D gate, §8 walk-forward sweep on 4H r9 (SOL+DOGE).
 *
 * Routed forward from TRA-308 (LeadDev FAIL report on TRA-298 + QuantTrader
 * decision in latest comment of TRA-298). Sibling child of TRA-291,
 * continuation of TRA-298 → TRA-297 → TRA-295 → TRA-287.
 *
 * Diff vs TRA-296 (`run-tra296-meanrev-long-regime-sweep.ts`)
 * ----------------------------------------------------------
 *
 *   1. Trigger family — REVERT to TRA-296 trigger combination
 *      (BB-reentry-long + ATR-pullback-long both active), but with **both
 *      triggers at TRA-295 §3/§4 spec, unchanged params** (loose
 *      ATR-pullback). Concretely vs TRA-296:
 *
 *        - BB-reentry-long: UNCHANGED (TRA-295 spec ≡ TRA-296 spec).
 *        - ATR-pullback-long: REVERTED from TRA-296 TIGHTENED (depth
 *          2.5×ATR, vol 1.5×, RSI(14)<40) back to TRA-295 LOOSE
 *          (depth 2.0×ATR, vol 1.2×, no RSI<40 filter).
 *
 *      Per TRA-308 spec §1: "Both triggers at the TRA-295 §3/§4 spec,
 *      unchanged params." Param-tuning of either trigger is explicitly
 *      out of scope for this iteration (see TRA-308 'Out of scope').
 *
 ...
 *      (most-recent-completed daily BTC.D close < SMA(synthetic BTC.D, 50)).
 *      Period stays 50 (TRA-298 confirmed local optimum on this universe;
 *      SMA(20) was strictly worse 1/9 vs TRA-296's 3/9).
 *
 *   3. NEW asset-side EMA(50) trend overlay — applied as an `OR` with the
 *      BTC.D gate. For each candidate trigger fire on bar `i` (4H), allow
 *      the entry if EITHER:
 *
 *        - **(existing BTC.D leg)** most-recent-completed daily synthetic
 *          BTC.D close < SMA(synthetic BTC.D, 50), OR
 *        - **(new EMA leg)** `close[i-1] > EMA(close, 50)[i-1]` on the
 *          entered asset itself (4H bars).
 *
 *      Rationale (per TRA-308 §3): dead-zone windows W2/W4/W7/W8 had
 *      synthetic BTC.D monotonically rising so the BTC.D leg never opens
 *      (no trailing MA on a monotone-up series ever flips below). The
 *      assets themselves may have had local mean-revertable pullbacks
 *      during those BTC-favorable quarters that asset-side trend filtering
 *      can admit without requiring BTC.D regime change.
 *
 *      OR (not AND) is deliberate — we are *adding* firing opportunity,
 *      not further restricting.
 *
 *      Note: ATR-pullback-long fires only when close[i-1] ≤ EMA50[i-1] -
 *      2.0×ATR(14)[i-1], which by definition implies close[i-1] < EMA50[i-1].
 *      So the EMA(50) leg can never admit an ATR-pullback signal — only
 *      BB-reentry-long benefits from this overlay.
 *
 ...
 *      XRP-aligned 9 walk-forward windows as TRA-287/292/294/295/297/298.
 *
 * §8 acceptance bar (UNCHANGED — task TRA-308 §7)
 * -----------------------------------------------
 *
 ...
 *
 * Reporting (per TRA-308 §3 + Deliverables)
 * -----------------------------------------
 *
 *   - Per-window §8 acceptance table (binding: trades + hit + expectancy + DD90).
 *   - Per-symbol per-window R decomposition.
 *   - Per-trigger pre-gate fire / post-gate-tradeable / gated histogram.
 *   - **NEW**: per-window per-asset breakdown of *entries by gate leg*:
 *     (BTC.D-only / EMA(50)-only / both / blocked).
 *   - Side-by-side TRA-296 vs TRA-299 per-window comparison so the
 *     asset-overlay effect is auditable (loads tra296 sidecar JSON).
 *
 * Run:
 *   pnpm --filter @trading-app/backtest exec tsx src/run-tra299-meanrev-long-bbatr-asset-overlay.ts
 *
 * Outputs:
 *   reports/tra299-sweep-4h-r9-meanrev-long-bbatr-asset-overlay.md
 *   reports/tra299-sweep-4h-r9-meanrev-long-bbatr-asset-overlay.json
 */
...
// ── Spec constants (task TRA-308 / file TRA-299) ──────────────────────────
...
// ATR-pullback-long trigger — TRA-295 §4 spec, REVERTED from TRA-296
// tightened. Loose params per TRA-308 §1.
const ATR_PB_PERIOD = 14;
const ATR_PB_EMA_PERIOD = 50;
const ATR_PB_DEPTH = 2.0;        // TRA-295: 2.0× (TRA-296 had 2.5×)
const ATR_PB_VOL_LOOKBACK = 20;
const ATR_PB_VOL_MULT = 1.2;     // TRA-295: 1.2× (TRA-296 had 1.5×)
// NOTE: no RSI(14)[i-1] < 40 filter — TRA-296 added it; reverted here.

// BTC.D regime gate — kept at SMA(50) per TRA-308 §2.
const BTC_D_SMA_PERIOD = 50;

// Asset-side EMA(50) trend overlay — TRA-308 §3. Same period as the
// ATR-pullback trigger's EMA reference (already pre-computed in
// `PreCompute.ema50`).
const ASSET_EMA_OVERLAY_PERIOD = 50;

// Walk-forward — UNCHANGED from TRA-287/292/294/295/297/298.
...
// Per-trigger post-gate "tradeable" counts (BTC.D OR EMA leg open).
...
// (leg, trigger) combination so we can compute btcd-only / ema-only / both
// from these aggregate counts.
const LEG_BTCD_ONLY_BB = 'leg: btcd-only (bb)';
const LEG_BTCD_ONLY_ATR = 'leg: btcd-only (atr)';
const LEG_EMA_ONLY_BB = 'leg: ema-only (bb)';
const LEG_EMA_ONLY_ATR = 'leg: ema-only (atr)';
...
interface WindowReport extends SimReport {
  ...
  testEnd: number;
  index: number;
  /** Per-symbol per-leg counts for the test span (per TRA-308 §3 reporting). */
...
/**
 * ATR-pullback-long — TRA-295 §4 spec, unchanged params.
 *
 * close[i-1] ≤ EMA50[i-1] − 2.0×ATR(14)[i-1] AND bullish reversal candle on
 * bar i (close>open AND close>close[i-1]) AND volume[i] > 1.2×mean(vol,20).
 * No RSI(14)[i-1] < 40 filter (that was a TRA-296 tightening, reverted here).
 */
...
function runLongSim(input: SimInput): SimReport {
  ...
  for (const ts of baseTimestamps) {
    ...
    for (const sym of symbols) {
      ...
      //   - emaLeg:  close[i-1] > EMA(close, 50)[i-1] on this asset (4H)
      ...
      const ema50Prev = idx > 0 ? pc.ema50[idx - 1] : NaN;
      const closePrev = idx > 0 ? bars[idx - 1].close : NaN;
      const emaLegReady = Number.isFinite(ema50Prev) && Number.isFinite(closePrev);
      const emaLegOpen = emaLegReady && (closePrev as number) > (ema50Prev as number);

      const tradeable = btcdLegOpen || emaLegOpen;

      // Per-trigger leg-attribution + post-gate / blocked accounting.
      // Pre-warm-up (BTC.D series < 50d) is bucketed separately (warm-up
      // missing) so we don't conflate "no signal" with "warm-up failed".
      function attribute(sigPresent: boolean, isBb: boolean) {
        if (!sigPresent) return;
        if (!btcdLegReady) {
          bumpSkip(SKIP_BTCD_WARMUP);
          // Even if BTC.D warm-up missing, EMA leg may still admit.
          if (emaLegOpen) {
            bumpSkip(isBb ? POST_GATE_BB : POST_GATE_ATR);
            bumpSkip(isBb ? LEG_EMA_ONLY_BB : LEG_EMA_ONLY_ATR);
          } else {
            bumpSkip(isBb ? BLOCKED_BB : BLOCKED_ATR);
          }
          return;
        }
        if (btcdLegOpen && emaLegOpen) {
          bumpSkip(isBb ? POST_GATE_BB : POST_GATE_ATR);
 ...
 *     counts aggregated across all symbols (drives the TRA-296-style histogram).
 *   - perSymbolLeg: per-symbol per-leg buckets (drives the new TRA-308 §3
 *     per-window per-asset reporting).
 *
...
function recomputeTestWindowSkipHistogram(
  ...
  for (const [sym, bars] of candlesBySymbol) {
    ...
    for (let i = 0; i < bars.length; i++) {
      ...
      const ema50Prev = i > 0 ? pc.ema50[i - 1] : NaN;
      ...
      function attribute(sigPresent: boolean, isBb: boolean) {
        ...
        if (!btcdLegReady) {
          ...
          if (emaLegOpen) {
            ...
            bump(isBb ? LEG_EMA_ONLY_BB : LEG_EMA_ONLY_ATR);
            if (isBb) legs.emaOnlyBb += 1; else legs.emaOnlyAtr += 1;
          } else {
...
function buildReport(
  ...
  const lines: string[] = [];
  lines.push('# TRA-299 — §8 Walk-Forward Sweep Report (4H Phase-1.2.3 BB+ATR + asset-side EMA(50) overlay supplementing SMA(50) BTC.D gate — r9 SOL+DOGE)\n');
  ...
  lines.push('## Triggers (TRA-299 deltas vs TRA-296)\n');
  ...
  lines.push('- **(EMA leg, NEW)** `close[i-1] > EMA(close, 50)[i-1]` on the entered asset itself (4H bars).');
  lines.push('');
  lines.push('Rationale (per TRA-308 §3): TRA-298 confirmed dead-zone windows W2/W4/W7/W8 had synthetic BTC.D monotonically rising — no trailing MA on a monotone-up series ever flips below — so the BTC.D-only gate never opens those quarters. The OR with an asset-side trend filter admits BB-reentry candidates during BTC-favorable quarters when the asset itself is in an uptrend. The OR (not AND) is deliberate — we *add* firing opportunity, not further restrict.');
  lines.push('');
  lines.push('Note: ATR-pullback fires only when `close[i-1] ≤ EMA(50)[i-1] − 2.0×ATR(14)[i-1]`, which by definition implies `close[i-1] < EMA(50)[i-1]`. So the EMA(50) leg can NEVER admit an ATR-pullback signal — only BB-reentry-long benefits from this overlay.');
  lines.push('');
  lines.push('Source — synthetic 5-coin basket loader (`./btc-dominance-synthetic.ts`), shared with TRA-296/297/298. BTC mcap divided by Σ(BTC + ETH + SOL + DOGE + XRP) mcaps using mid-period 2024 circulating-supply constants from existing daily Coinbase caches. Reproducible offline, no API key required.');
  lines.push('');
  ...
  lines.push('Asset-side EMA(50) warm-up: each window\'s slice covers train(1080×4H) + test(540×4H) = 1620 bars per asset, well above the 50-bar warm-up requirement. The EMA leg is fully warm at every window\'s `testStart`. Confirmed.');
  lines.push('');
  ...
  lines.push('');
  lines.push('Notes: per-trigger pre-gate counts every signal fire (BB and ATR are independent). Post-gate-tradeable = `(BTC.D leg open) OR (asset-side EMA(50) leg open)` AND BTC.D warm-up satisfied (or EMA leg alone admits if warm-up missing). Blocked = BTC.D warm-up satisfied AND neither leg open.');
  lines.push('');

  // NEW: per-window per-asset gate-leg attribution histogram.
  lines.push('## Per-window per-asset gate-leg attribution (TRA-308 §3 reporting)\n');
  lines.push('Per-trigger fire counts bucketed by which leg(s) of the OR-gate were open on the trigger bar. Counts are independent per trigger (BB and ATR can both fire on the same bar). Pre-gate = BTCD-only + EMA-only + Both + Blocked + warm-up missing.');
  lines.push('');
  lines.push('| Window | Asset | BB pre-gate | BB BTCD-only | BB EMA-only | BB Both | BB Blocked | ATR pre-gate | ATR BTCD-only | ATR EMA-only | ATR Both | ATR Blocked |');
  lines.push('| ------ | ----- | ----------- | ------------ | ----------- | ------- | ---------- | ------------ | ------------- | ------------ | -------- | ----------- |');
  for (const w of wf.windows) {
    for (const psl of w.perSymbolLeg) {
      const l = psl.legs;
      const bbPre = l.btcdOnlyBb + l.emaOnlyBb + l.bothBb + l.blockedBb;
      const atrPre = l.btcdOnlyAtr + l.emaOnlyAtr + l.bothAtr + l.blockedAtr;
      lines.push(`| ${w.index} | ${psl.symbol} | ${bbPre} | ${l.btcdOnlyBb} | ${l.emaOnlyBb} | ${l.bothBb} | ${l.blockedBb} | ${atrPre} | ${l.btcdOnlyAtr} | ${l.emaOnlyAtr} | ${l.bothAtr} | ${l.blockedAtr} |`);
    }
  }
  lines.push('');
  lines.push('Reading guide:');
  lines.push('- **EMA-only** counts are the new opportunity the asset-overlay opens (BB-reentries that BTC.D-only would have blocked).');
  lines.push('- ATR EMA-only is structurally always 0 (the trigger requires close[i-1] < EMA50[i-1]).');
  lines.push('- A window where BB EMA-only ≈ BB Blocked-pre-overlay tells us the overlay rescued the trigger; if EMA-only is small or zero, the overlay was inert in that window.');
  lines.push('');
  ...
  // Side-by-side TRA-296 vs TRA-299.
  lines.push('## Side-by-side TRA-296 vs TRA-299 (per window)\n');
  lines.push('| Window | TRA-296 trades | TRA-299 trades | Δ trades | TRA-296 hit% | TRA-299 hit% | TRA-296 R | TRA-299 R | TRA-296 DD90% | TRA-299 DD90% | TRA-296 pass? | TRA-299 pass? |');
  lines.push('| ------ | -------------- | -------------- | -------- | ------------ | ------------ | --------- | --------- | ------------- | ------------- | ------------- | ------------- |');
  for (const w of wf.perWindow) {
    const m = w.metrics;
  ...
  lines.push('## Decision\n');
  if (passingCount >= 5) {
    lines.push(`**SHIP SIGNAL** — ${passingCount} / 9 windows clear all four binding §8 bars (≥ 5 / 9 required), with the 25% rolling-90d DD bar binding. Asset-side EMA(50) overlay supplementing SMA(50) BTC.D gate clears the bar.`);
  } else {
    lines.push(`**FAIL** — Only ${passingCount} / 9 windows clear all four binding §8 bars (≥ 5 / 9 required). Per task spec, do **not** silently tune. Reassign back to QuantTrader with the per-leg fire counts (above), per-window working-set comparison vs TRA-296, and W3 status. Possible follow-ups (QuantTrader call, in priority order):`);
    lines.push('1. **Different regime proxy** — TOTAL3/TOTAL ratio (excluding BTC, ETH, stables), or asset-only proxy `close > EMA(200) AND slope(EMA(50)) > 0`.');
    lines.push('2. **Different trigger family** — RSI bullish divergence, VWAP-revert intraday. Only if BB+ATR with overlay aggregates negative.');
    lines.push('3. **Accept partial-coverage acceptance** — concede mean-rev long is a 5/9-quarter-coverage strategy on SOL+DOGE 4H r9; universe needs a complementary BTC-favorable strategy. (Scope-redefinition decision, not a tuning iteration.)');
  }
  return lines.join('\n');
}
...
async function main() {
  ...
  console.log(`[run-tra299] Loading 4H bars ${fmtDate(fromMs)} → ${fmtDate(toMs)} for r9 universe ${UNIVERSE.join(', ')}`);
  ...
  console.log(`[run-tra299] Aligned to ${new Date(ALIGNED_START_TS).toISOString()}: ${aligned}`);

  console.log('[run-tra299] Loading synthetic BTC.D daily series (5-coin basket, shared loader)…');
  ...
  console.log(`[run-tra299] BTC.D series: ${dominance.points.length} points, ${fmtDate(minDom.ts)} → ${fmtDate(maxDom.ts)}`);
  const lookupAtAligned = dominance.lookup(ALIGNED_START_TS);
  console.log(`[run-tra299] BTC.D snapshot @ aligned start ${fmtDate(ALIGNED_START_TS)}: dominance=${lookupAtAligned?.dominance.toFixed(3) ?? 'n/a'}, sma=${lookupAtAligned?.sma?.toFixed(3) ?? 'n/a'}`);

  console.log('[run-tra299] Running BB+ATR + asset-side EMA(50) overlay walk-forward…');
  const wf = runWalkForward(fullByPair, dominance);
  console.log(`[run-tra299] ${wf.windows.length} windows produced.`);

  const symbols = [...UNIVERSE];
  const tra296Prior = maybeLoadTra296Sidecar();
  const report = buildReport(wf, symbols, fullByPair, dominance, tra296Prior);
  mkdirSync(REPORT_DIR, { recursive: true });
  const reportPath = resolve(REPORT_DIR, 'tra299-sweep-4h-r9-meanrev-long-bbatr-asset-overlay.md');
  writeFileSync(reportPath, report);
  console.log(`\n[run-tra299] Report written: ${reportPath}`);
  ...
  const sidecarPath = resolve(REPORT_DIR, 'tra299-sweep-4h-r9-meanrev-long-bbatr-asset-overlay.json');
  writeFileSync(sidecarPath, JSON.stringify({
    generatedAt: new Date().toISOString(),
    granularity: '4h',
    direction: 'long',
    triggerFamily: 'mean-reversion (BTC.D OR asset-EMA50 gate)',
    ...
    atrPullbackRsiMax: null,
    btcdSmaPeriod: BTC_D_SMA_PERIOD,
    btcdSource: 'synthetic-5coin-basket',
    btcdBasket: BASKET,
    assetEmaOverlayPeriod: ASSET_EMA_OVERLAY_PERIOD,
    gateCombination: 'OR (BTC.D leg OR asset-side EMA(50) leg)',
    ...
    perWindow: wf.perWindow.map((w) => {
      ...
      return {
        ...
        legCounts: {
          ...
          emaOnlyBb: r.get(LEG_EMA_ONLY_BB) ?? 0,
          emaOnlyAtr: r.get(LEG_EMA_ONLY_ATR) ?? 0,
  ...
  console.log(`[run-tra299] JSON sidecar: ${sidecarPath}`);

  console.log(`\n=== TRA-299 §8 Acceptance (4H r9 BB+ATR + asset-side EMA(50) overlay, SOL+DOGE) ===`);
  console.log(`Windows passing all four binding bars: ${passingCount} / ${wf.perWindow.length}`);
  console.log(`Decision: ${passAll ? 'PASS — ship signal' : 'FAIL — TRA-299 → QuantTrader'}`);
}

const invoked = process.argv[1] && /[\\/]run-tra299-meanrev-long-bbatr-asset-overlay\.(ts|js)$/.test(process.argv[1]);
